...
export default class LR0Viewer extends React.Component<LR0ViewerProps, LR0ViewerState> {
    ...
    createNewTable () {
        ...
        const strType = this.props.type == 0 ? "LR(0)" : (this.props.type == 1 ? "LR(1)" : "LALR(1)")

        const result = this.props.Grammar.parseStringWithLR(this.state.testString, this.props.type != 0, this.props.type == 2, (stackContent, position, action) => {
            newTable.push(
                <tr key={`row ${row++}`}>
                    <td>
                        {
        ...
        if(result == null){
            M.toast({html: `Not a ${strType} grammar`})
    ...
    render () {
        ...
        return (
            <div>
                <h4 className="blue-grey-text text-darken-4">{strType} parser using {this.props.Grammar.getName()}</h4>
